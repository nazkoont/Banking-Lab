...
DEEPER_DIR   = "Data_wayback/html/deeper"
...
def main():
    ...
    # collect from both products and deeper disclosure folders
    all_files = collect_html_files(PRODUCTS_DIR, args.company)
    if os.path.isdir(DEEPER_DIR):
        all_files += collect_html_files(DEEPER_DIR, args.company)
    if page_type_filter:
        filtered = []
        for f in all_files:
    ...
    if os.path.exists(master_csv):
        ...
        with open(master_csv, newline="", encoding="utf-8") as mf:
            ...
            for row in reader:
                ...
                company_folder_deeper = os.path.join(DEEPER_DIR, company_slug)
                # skip if company has no scraped folder in either source
                if not os.path.isdir(company_folder) and not os.path.isdir(company_folder_deeper):
                    continue
