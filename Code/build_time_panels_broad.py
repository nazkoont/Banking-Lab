...
# has_baas, n_with_baas, has_X_baas still use specific only to avoid overcounting
# n_baas_partners counts unique broad bank partners
rows = []
for (company, period), grp in df.groupby(['company', 'period']):

    # still specific for has_baas/n_with_baas to avoid overcounting
    ...
    row = {
        ...
        'n_baas_partners':       grp['bank_partner_broad'].dropna().nunique(),
        'baas_partners':         '; '.join(grp['bank_partner_broad'].dropna().unique()),
...
firm_panel.to_csv('Data_cleaned/firm_time_panel_broad.csv', index=False)
print(f"Firm broad: {len(firm_panel)} rows, {firm_panel['company'].nunique()} companies")

# ── BANK PANEL (broad) ────────────────────────────────────────────────────────
# one row per (bank, half-year period)
# simpler than specific version: just firm counts, no subcategories
...
bank_rows = df[df['bank_partner_broad'].notna()].copy()
bank_rows['bank_norm'] = bank_rows['bank_partner_broad'].apply(normalize_bank)
...
bank_panel.to_csv('Data_cleaned/bank_time_panel_broad.csv', index=False)
print(f"Bank broad: {len(bank_panel)} rows, {bank_panel['bank_name'].nunique()} banks")
